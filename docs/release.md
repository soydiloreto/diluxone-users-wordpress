...
For maintainers. End users get the plugin from [wordpress.org/plugins/diluxone-users](https://wordpress.org/plugins/diluxone-users/) — they don't need to read this document.
...
The PHP `Version:` header in `diluxone-users.php` and the `DILUXONE_USERS_VERSION` constant carry a `-dev` suffix on `main` between releases. The `Stable tag:` in `readme.txt` does **not** — it always holds the last published release, or before any release the next intended one.

| State | PHP `Version:` | `DILUXONE_USERS_VERSION` | readme `Stable tag:` |
| --- | --- | --- | --- |
...
2. **Open a release-prep PR.** Branch name: `chore/release-X.Y.Z`. The PR does three things:
   - Drops the `-dev` suffix in `diluxone-users.php` (the `Version:` header **and** the `DILUXONE_USERS_VERSION` constant).
...
6. **The deploy workflow takes it from there.** [`.github/workflows/deploy.yml`](../.github/workflows/deploy.yml) fires on tag push; it does:
   ...
   - Strict version-alignment of all three markers — PHP `Version:`, `DILUXONE_USERS_VERSION`, readme `Stable tag:` — against the git tag.
   - [`10up/action-wordpress-plugin-deploy@stable`](https://github.com/10up/action-wordpress-plugin-deploy) to push `/trunk` and tag `/tags/X.Y.Z` on the wp.org SVN, and to upload `.wordpress-org/` assets to the SVN `/assets/` directory. It is given `SLUG: diluxone-users` explicitly: the action defaults that to the repository name, which here is `diluxone-users-wordpress` and would target an SVN path that does not exist.
   - Generate the GitHub release with the changelog excerpt as the body.

7. **Verify on wp.org** within ~10 minutes. The new version should appear at `https://wordpress.org/plugins/diluxone-users/`. wp.org does not run automated rollouts — sites with auto-update enabled pick it up over the next ~12 hours via the WordPress core update check.

8. **Bump back to dev.** Open a follow-up PR `chore/bump-(X.Y.Z+1)-dev` that:
   - Sets PHP `Version:` and `DILUXONE_USERS_VERSION` to `(next intended version)-dev`.
